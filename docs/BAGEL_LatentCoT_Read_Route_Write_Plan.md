...
| C1 | 去掉 | **关** | `[16,24)`, R=1 | off | strict read-only causal control |
| C2 | 保留 | **关** | `[16,24)` | off | old-prompt-only control |
...
| read-only 是否与 vanilla 隔离？ | Z0 vs C1 |  |
| historical old loop 与主候选差异？ | Z1 vs Z2（多变量 reference） |  |
| mid-layer 是否更适合？ | Z2/Z3/Z4 |  |
| old prompt 是否阻碍编辑？ | Z2 vs C2 |  |
...
Phase 0.5 的 paired semantic-edit evaluation harness 使用
`experiments/data/semantic_edit_phase05.jsonl`，每条样本显式记录 source image、
source prompt、instruction、target 与 preserve constraints。仓库暂不包含 source
images，也未接 target/preserve evaluator，所以它是实验规格与 runner，不是
quantitative benchmark。主矩阵固定为 Z0–Z5/C0–C2；K 消融独立报告
Z0/K={1,4,8}，不与主矩阵混合。
